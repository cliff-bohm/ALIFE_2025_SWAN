...
auto makeTemplateBrain(int inputs, int outputs, std::shared_ptr<ParametersTable> PT) -> std::shared_ptr<AbstractBrain> {
  ...
  if (brainType == "Chris") {
    newBrain = ChrisBrain_brainFactory(inputs, outputs, PT);
    found = true;
    }
  if (!found){
...
auto makeWorld(std::shared_ptr<ParametersTable> PT) -> std::shared_ptr<AbstractWorld> {
  ...
  if (worldType == "VJ2") {
    newWorld = std::make_shared<VJ2World>(PT);
    found = true;
    }
  if (!found){
...
void configureDefaultsAndDocumentation(){
  Parameters::root->setParameter("BRAIN-brainType", (std::string)"Chris");
  Parameters::root->setDocumentation("BRAIN-brainType", "brain to be used, [Chris]");
  ...
  Parameters::root->setParameter("WORLD-worldType", (std::string)"VJ2");
  Parameters::root->setDocumentation("WORLD-worldType","world to be used, [Berry, VJ2]");
}
